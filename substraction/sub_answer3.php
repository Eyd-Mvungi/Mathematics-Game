  ...
  for($i = 0; $i < $totalQuestions; $i++) {
    ...
    if ($countRight){
      ...
      $sql = "INSERT INTO `result`(`idscore`, `student_student_id`, `calculation_calculation_id`, `level_level_id`, `total_mark`, `grade`, `status`)
         VALUES ('','$data','2','3','$countRight','$grade','$status')";
  ...
  <div id="redForm">
    <h1 class=""><table style="">Submitted answers for the attempted questions, you have scored <?=$countRight." % ". "(".$grade.")".  $status?></h1>
      ...
      <?php for($i=0; $i < $totalQuestions; $i++) {?>
        ...
        <li><?=$operand1[$i] . " - " . $operand2[$i] . " - " . $operand3[$i] .  " = " . $jibu[$i] . ""?>
    ...
    <?php for($i=0; $i < $totalQuestions; $i++) {?>
      ...
      <li><?=$operand1[$i] . " - " . $operand2[$i] . " - " . $operand3[$i] .  " = " . $sub[$i] . ""?></li>
...
<button class="button button1" ><a href="nextpage/substratctionlevels.php" style="text-decoration: none;"> <h3>Perfom More substraction</h3></a></button>
</div>
