...
for ($question = 1; $question <= $totalQuestions; $question++){
  ...
  if ($countRight){
    ...
    $sql = "INSERT INTO `result`(`idscore`, `student_student_id`, `calculation_calculation_id`, `level_level_id`, `total_mark`, `grade`, `status`)
       VALUES ('','$data','3','4','$countRight','$grade','$status')";
  ...
  <div id="redForm">
    <h1 class=""><table style="">Submitted answers for the attempted questions, you have scored : <?=$countRight." % ". "(".$grade.")".  $status?></h1>
      ...
      <?php for($i=0; $i < $totalQuestions; $i++) {?>
        ...
        <li><?=$operand1[$i] . "  x  " . $operand2[$i] ."  x  " . $operand3[$i] ."  x  " . $operand4[$i] . " = " . $jibu[$i] . ""?>
    ...
    <?php for($i=0; $i < $totalQuestions; $i++) {?>
      ...
      <li><?=$operand1[$i] . "  x  " . $operand2[$i] . "  x  " . $operand3[$i] ."  x  " . $operand4[$i] ." = " . $sum[$i] . ""?></li>
...
<button class="button button1" ><a href="nextpage/indexmult.html" style="text-decoration: none;"> <h3>Perfom More multiplication</h3></a></button>
</div>
